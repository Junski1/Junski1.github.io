 ...
 */
const path = "../img/";
const infoImg = "info200.png";
const videoImg = "videoIcon.png";
const images = [
    {
        name: "Sludge",
        imgs: 
    ...
    {
        name: "Return to Metsola",
        imgs:
        [
            {src:"ReturnToMetsola/Mainmenu.png", alt: "Home screen"},
            {src:"ReturnToMetsola/Gameplay.png", alt: "Gameplay"},
            {src:"ReturnToMetsola/DeathScreen.png", alt: "Death screen"},
            {src:"ReturnToMetsola/info.txt", alt: "info"}
        ]
    },
    {
        name: "Visual Novel",
...
async function CreateProjectsPage() {
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        newContent.setAttribute('src', `${path}${images[i].imgs[0].src}`);
        newContent.setAttribute('onclick', `OpenModal(${i+1});CurrentSlide(${1})`);

        parent.appendChild(newContent);

        newContent = document.createElement("div");
        newContent.classList.add("caption-container","nameText");
        newContent.innerText = images[i].name;
        ...
        for(let j = 0; j < images[i].imgs.length; j++) {
            ...
            newContent.innerText = `${j+1} / ${images[i].imgs.length}`;

            parent.appendChild(newContent);

            if(images[i].imgs[j].alt == "video"){

                newContent = document.createElement("iframe");
                newContent.setAttribute("src", `${images[i].imgs[j].src}`);
                newContent.style.height = "600px";

                parent.appendChild(newContent);

            }
            else if (images[i].imgs[j].alt == "info"){

                var info;
                await fetch(`${path}${images[i].imgs[j].src}`)
                            .then((res) => res.json()
                            .then((text) => {
                                info = text;
                ...
                newContent = document.createElement("img");
                newContent.setAttribute('src', `${path}${images[i].imgs[j].src}`);
                parent.appendChild(newContent);
            ...
            for(let j = 0; j < images[i].imgs.length; j++) {
                ...
                if(images[i].imgs[j].alt == "info")
                    newContent.setAttribute("src", `${path}${infoImg}`);
                else if(images[i].imgs[j].alt == "video")
                    newContent.setAttribute("src", `${path}${videoImg}`);
                else
                    newContent.setAttribute("src", `${path}${images[i].imgs[j].src}`);

                newContent.setAttribute("onclick", `CurrentSlide(${j+1})`);
                newContent.setAttribute("alt", `${images[i].imgs[j].alt}`);
                parent.appendChild(newContent);
            } 
    }
}
